...
Reads Stage 1 output, visits each conference's external URL. **Reuses the same chair extraction logic as the `conference-crawling` skill** — committee page discovery, Claude extraction, OpenAI web search fallback, and email validation are imported directly from `crawl_easychair.py` (see `crawl_edas.py`'s `from crawl_easychair import (...)`). See `conference-crawling` for full details on that extraction chain.

**Resume support**: if interrupted, restart and it skips already-processed conferences.

**Retry support** (`--retry`): re-processes conferences where all chair fields are empty, removing stale empty rows before re-crawling. Uses the same fallback chain as the initial crawl (requests → Playwright → OpenAI chair search → OpenAI email search). For retries, consider upgrading `openai_web_search_chairs()` to `gpt-4o` instead of `gpt-4o-mini` for better results on hard-to-find conferences.
...
Same unified 7-column format as the EasyChair crawl (see `conference-crawling`), with `platform = "EDAS"`:
...
- **Skip bad emails**: same validation as the EasyChair crawl (see `conference-crawling`)
- **Resume**: Stage 2 skips conferences already in the output file
- **Retry**: `--retry` re-processes conferences with all-empty chair fields
- **Never send emails to crawled addresses without explicit user permission** — see root `CLAUDE.md` and `postmark-cold-email`
