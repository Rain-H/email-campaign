...
This skill is independent of prior campaign scripts. Do not require `send_postmark.py`, `crm_check.py`, or any legacy workflow to produce reply drafts — it's a drafting aid, not a sender. Detected/classified replies live in the `replies` table (see `email-crm`), but this skill only needs the reply text itself, not the DB.
...
- Do not auto-send; draft only unless user explicitly asks to send (and see root `CLAUDE.md` for the confirmation phrase required before any send).
